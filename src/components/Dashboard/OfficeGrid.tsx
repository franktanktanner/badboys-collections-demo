...
import { officeStats } from '../../data/mockAccounts';
...
export function OfficeGrid() {
  ...
  const sorted = [...officeStats].sort((a, b) =>
        ...
        {sorted.map((o, i) => {
          ...
          return (
            <motion.div
              ...
              className="group cursor-pointer rounded-lg border border-border bg-bg-elevated/40 p-4 transition-all hover:border-brand-gold/30 hover:bg-bg-elevated/80"
            >
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-2">
                  <MapPin className="h-3.5 w-3.5 text-brand-gold" />
                  <span className="text-sm font-medium text-white">{o.office}</span>
                </div>
                <span className={cn(
